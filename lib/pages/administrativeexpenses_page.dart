...
class AdministrativeExpenses extends StatefulWidget {
  final int projectId;
  const AdministrativeExpenses({Key? key, required this.projectId}) : super(key: key);

  @override
  State<AdministrativeExpenses> createState() => _AdministrativeExpensesState();
}

class _AdministrativeExpensesState extends State<AdministrativeExpenses> {
  ...
  Future<void> fetchAdministrativeExpenses() async {
    var apiService = ApiServices();
    var response = await apiService.getAdministrativeexpenses(widget.projectId);
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(
          ...
          return AlertDialog(
            title: const Text('Add New Administrative expenses'),
            content: Container(
            ...
            actions: <Widget>[
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    // Default values applied if fields are left empty
                    ...
                    if (_descriptionController.text.isNotEmpty) {
                      ...
                      var response = await _apiServices.addAdministrativeExpense(newExpense, widget.projectId);
                      if (response != null) {
                        // Assuming response contains the added expense details.
                        Expense addedExpense = Expense.fromJson(response.toJson());
                        expenses.add(addedExpense);
                        // You might need to ensure this is called on the main thread or use appropriate state management
                        setState(() {});
                        await fetchAdministrativeExpenses();
                        await fetchProjectTotals();
                      }
                      Navigator.of(context).pop();
                    } else {
                      // Optionally show an alert or a snackbar message indicating that the description cannot be empty
                    }
                  },
    ...
    return  isMobile(context) ?  Expanded(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
             Text(
              "Add New Administrative Expenses",
      ...
      child: Column(
        children: [
          Text(
            ...
            style: TextStyle(fontSize: MediaQuery.of(context).size.width * 0.06, fontWeight: FontWeight.w500),
          ),
          ...
          child: Scrollbar(
            ...
            child: SingleChildScrollView(
              ...
              child: DataTable(
                ...
                rows: expenses.asMap().entries.map(
                      (entry) {
                    final expense = entry.value;  // Corrected here
                    return DataRow(
                      cells: [
                        ...
                        DataCell(Text('${expense.amount ?? '0'} EGP')),
                        DataCell(Text('${expense.dollarAmount ?? '0'} \$')),
                        DataCell(Text('${expense.rate ?? '0'}')),
                        DataCell(Text('${expense.total ?? '0'} EGP')),
                        DataCell(Text(expense.username ?? 'N/A')),
                        DataCell(Text(formatUtcToLocalDateString(expense.date))),
                        DataCell(
                          Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              IconButton(
                                ...
                                onPressed: () => _deleteExpense(expense.expenseId, entry.key),
                              ),
                            ],
                          ),
                        ),
  ...
  Future<void> _deleteExpense(int? expenseId, int index) async {
    if (expenseId == null) return;

    final response = await _apiServices.deleteAdministrativeExpense(expenseId);
