...
class PurchaseInvoices extends StatefulWidget {
  final int projectId;
  const PurchaseInvoices({Key? key, required this.projectId}) : super(key: key);
...
class _PurchaseInvoicesState extends State<PurchaseInvoices> {
  ...
  void initState() {
    super.initState();
    fetchgetPurchaseInvoices();
    fetchProjectTotals();
  }

  Future<void> fetchgetPurchaseInvoices() async {
    var apiService = ApiServices();
    var response = await apiService.getPurchaseInvoices(widget.projectId);

    if (response != null && response.expensespi != null) {
      setState(() {
        expensespi = response.expensespi;
        isLoading = false;
      });
    }
  }
        ...
        builder: (BuildContext context) {
          return StatefulBuilder(
            builder: (context, setModalState) {
              return AlertDialog(
          title: const Text('Add New Purchase Invoices'),
          content: Container(
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  var response = await _apiServices.addPurchaseInvoices(newExpense, widget.projectId);

                  if (response != null) {
                    // Assuming response contains the added expense details.
                    ExpensePi addedExpense = ExpensePi.fromJson(response.toJson());
                    expensespi.add(addedExpense);
                    setState(() {});
                   await fetchgetPurchaseInvoices() ;
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              Text(
                " ${projectTotals?.receiptOfBuyTotal ?? 'Loading...'} EGP",
                style: TextStyle(fontWeight: FontWeight.w500, fontSize: 15),
              ),
            ],
          ),
          ...
          child: Scrollbar(
            ...
            child: SingleChildScrollView(
              ...
              child: DataTable(
                ...
                rows: expensespi.asMap().entries.map(
                      (entry) {
                    final expensepi = entry.value;  // Corrected here
                    return DataRow(
                      cells: [
                        ...
                        DataCell(Text('${expensepi.amount ?? '0'} EGP')),
                        DataCell(Text('${expensepi.dollarAmount ?? '0'} \$')),
                        DataCell(Text('${expensepi.rate ?? '0'}')),
                        DataCell(Text('${expensepi.total ?? '0'} EGP')),
                        DataCell(Text(expensepi.username ?? 'N/A')),
                        DataCell(Text(formatUtcToLocalDateString(expensepi.date))),
                        DataCell(
                          Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              IconButton(
                                ...
                                onPressed: () => _deleteExpense(expensepi.expenseIdip, entry.key),
                              ),
                            ],
                          ),
                        ),
  ...
  Future<void> _deleteExpense(int? expenseIdip, int index) async {
    if (expenseIdip == null) return;

    final response = await _apiServices.deleteReceiptOfBuy(expenseIdip);
    if (response?.message != null) {
      setState(() {
        expensespi.removeAt(index);
      });
      await fetchProjectTotals();
    } else {
